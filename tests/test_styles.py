...
def test_style_registry_loads_prompt_files(tmp_path: Path) -> None:
    ...
    prompts.mkdir()
    (prompts / "work-chat.md").write_text("Make it polite.", encoding="utf-8")
    (prompts / "ignored.txt").write_text("Ignore non-Markdown.", encoding="utf-8")
    (prompts / "writing").mkdir()
    (prompts / "writing" / "email.md").write_text("Write as email.", encoding="utf-8")
    (prompts / "writing" / "README.md").write_text("Nested docs only.", encoding="utf-8")
    (prompts / ".hidden").mkdir()
    (prompts / ".hidden" / "secret.md").write_text("Ignore me.", encoding="utf-8")
    ...
    assert nested_style.label == "email"
